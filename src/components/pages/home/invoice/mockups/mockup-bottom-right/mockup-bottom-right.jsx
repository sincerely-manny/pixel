import whiteNoise from '../../images/bg-white-noise.png';

const MockupBottomRight = () => (
  <svg
    width="410"
    height="605"
    viewBox="0 0 410 605"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className="pointer-events-none absolute bottom-0 left-0 h-full w-full shadow-xl"
    preserveAspectRatio="none"
  >
    <g opacity="1">
      <mask
        id="mask0_10_47"
        className="[mask-type:alpha]"
        maskUnits="userSpaceOnUse"
        x="-1"
        y="0"
        width="411"
        height="606"
      >
        <g opacity="0.15">
          <path
            d="M410 3.00196C410 1.34511 408.656 0.00195312 407 0.00195312H2.99963C1.34278 0.00195312 -0.000366211 1.3451 -0.000366211 3.00195V602.002C-0.000366211 603.659 1.34278 605.002 2.99963 605.002H407C408.656 605.002 410 603.659 410 602.002V3.00196Z"
            fill="url(#paint0_linear_10_47)"
          />
        </g>
      </mask>
      <g mask="url(#mask0_10_47)">
        <g className="mix-blend-overlay">
          <path
            d="M409.5 3.00196C409.5 1.62125 408.38 0.501953 407 0.501953H2.99962C1.61891 0.501953 0.499622 1.62124 0.499622 3.00195V602.002C0.499622 603.383 1.61895 604.502 2.99962 604.502H407C408.38 604.502 409.5 603.383 409.5 602.002V3.00196Z"
            fill="url(#paint1_radial_10_47)"
            fillOpacity="0.8"
          />
          <path
            className="mix-blend-overlay"
            d="M409.5 3.00196C409.5 1.62125 408.38 0.501953 407 0.501953H2.99962C1.61891 0.501953 0.499622 1.62124 0.499622 3.00195V602.002C0.499622 603.383 1.61895 604.502 2.99962 604.502H407C408.38 604.502 409.5 603.383 409.5 602.002V3.00196Z"
            fill="url(#pattern0)"
            fillOpacity="0.1"
          />
          <path
            d="M409.5 3.00196C409.5 1.62125 408.38 0.501953 407 0.501953H2.99962C1.61891 0.501953 0.499622 1.62124 0.499622 3.00195V602.002C0.499622 603.383 1.61895 604.502 2.99962 604.502H407C408.38 604.502 409.5 603.383 409.5 602.002V3.00196Z"
            stroke="url(#paint2_linear_10_47)"
          />
        </g>
        <g className="mix-blend-overlay" opacity="0.4">
          <path
            d="M409.5 3.00196C409.5 1.62125 408.38 0.501953 407 0.501953H2.99962C1.61891 0.501953 0.499622 1.62124 0.499622 3.00195V602.002C0.499622 603.383 1.61895 604.502 2.99962 604.502H407C408.38 604.502 409.5 603.383 409.5 602.002V3.00196Z"
            stroke="url(#paint3_linear_10_47)"
          />
        </g>
      </g>
    </g>
    <defs>
      <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
        <use xlinkHref="#image0_10_47" transform="scale(0.00215517 0.00144509)" />
      </pattern>
      <linearGradient
        id="paint0_linear_10_47"
        x1="410"
        y1="140.002"
        x2="-0.000366211"
        y2="140.002"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="white" />
        <stop offset="0.182927" stopColor="white" stopOpacity="0" />
      </linearGradient>
      <radialGradient
        id="paint1_radial_10_47"
        cx="0"
        cy="0"
        r="1"
        gradientUnits="userSpaceOnUse"
        gradientTransform="translate(409.601 0.438865) rotate(124.125) scale(730.839 474.106)"
      >
        <stop stopColor="white" />
        <stop offset="0.741521" stopColor="white" stopOpacity="0" />
      </radialGradient>
      <linearGradient
        id="paint2_linear_10_47"
        x1="409.075"
        y1="-4.00886"
        x2="-0.00432773"
        y2="395.002"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset="0.276476" stopColor="white" />
        <stop offset="0.867466" stopColor="white" stopOpacity="0" />
      </linearGradient>
      <linearGradient
        id="paint3_linear_10_47"
        x1="409.075"
        y1="-4.00886"
        x2="-0.00432773"
        y2="395.002"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset="0.276476" stopColor="white" />
        <stop offset="0.867466" stopColor="white" stopOpacity="0" />
      </linearGradient>
      <image id="image0_10_47" width="464" height="692" xlinkHref={whiteNoise.src} />
    </defs>
  </svg>
);

export default MockupBottomRight;
